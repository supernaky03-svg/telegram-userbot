...
POLL_INTERVAL_SECONDS = 3600   # 1 hour backup polling
LATEST_RECHECK_LIMIT = 10
    ...
    try:
        with open(USER_DATA_FILE, "r", encoding="utf-8") as f:
            ...
            if not isinstance(data, dict):
                logger.warning("user_data.json invalid root type. Resetting.")
                return {}
            return data
    except json.JSONDecodeError:
        backup_name = f"{USER_DATA_FILE}.corrupt"
        try:
            shutil.copy(USER_DATA_FILE, backup_name)
        except Exception:
            logger.exception("Failed to back up corrupted JSON.")
        logger.exception("JSON corruption detected. Resetting data.")
        return {}
    except Exception:
...
def save_user_data() -> None:
    try:
        ...
        logger.exception("Failed to save user_data.json")


def normalize_user_record(record: Dict[str, Any]) -> Dict[str, Any]:
    if "pairs" not in record:
        channel_a = record.get("channel_a", "")
        channel_b = record.get("channel_b", "")
        last_processed_id = record.get("last_processed_id", 0)
        recent_sent_ids = record.get("recent_sent_ids", [])
        record = {
            "pairs": []
        }
        if channel_a or channel_b:
            record["pairs"].append({
                ...
                "recent_sent_ids": recent_sent_ids[-RECENT_IDS_LIMIT:],
            })
    if "pairs" not in record or not isinstance(record["pairs"], list):
        record["pairs"] = []
    return record
...
def ensure_user_record(user_id: int) -> Dict[str, Any]:
    ...
    if key not in user_data:
        user_data[key] = {"pairs": []}
        save_user_data()
    user_data[key] = normalize_user_record(user_data[key])
...
def mark_sent_ids(pair: Dict[str, Any], source_ids: List[int]) -> None:
...
def update_last_processed(pair: Dict[str, Any], msg_id: int) -> None:
    ...
    async with runtime["lock"]:
        try:
            if getattr(msg, "grouped_id", None):
                update_last_processed(pair, msg.id)
                return
            ...
            update_last_processed(pair, getattr(msg, "id", 0))
...
async def process_album_object(user_id: int, pair: Dict[str, Any], album_messages: List[Any]) -> None:
    ...
    async with runtime["lock"]:
        try:
            ...
            if not has_video:
                update_last_processed(pair, highest_id)
                return
